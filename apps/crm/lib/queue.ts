...
async function processReceipt(job: Job<ReceiptEvent>) {
  console.log(`[worker] processing ${job.data.event} for ${job.data.messageId}`);
...
if (connection && !globalWorker.receiptWorker) {
...
export function startReceiptWorker() {
  // Worker is started automatically on module import
}
